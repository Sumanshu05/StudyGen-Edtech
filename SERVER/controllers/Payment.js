...
exports.verifyPayment = async (req, res) => {
  ...
  if (expectedSignature === razorpay_signature) {
    await enrollStudents(courses, userId, res, razorpay_order_id, razorpay_payment_id);
...
// ================= ENROLL STUDENTS =================
const enrollStudents = async (courses, userId, res, orderId, paymentId) => {
  if (!courses || !userId) {
    return res
      .status(400)
  ...
  for (const courseId of courses) {
    try {
      ...
      // Record purchase separately (original behavior)
